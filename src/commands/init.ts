...
export function initCommand(program: Command): void {
  program
    ...
    .action(async (options: CommandOptions) => {
      console.log(
        chalk.blue("Initializing a new Specky component specification..."),
      );
      ...
      try {
          ...
          const { overwrite } = await inquirer.prompt([
            {
              ...
              name: "overwrite",
              message:
                "A spec.json file already exists. Do you want to overwrite it?",
              default: false,
            },
          ]);
        ...
        const answers = await promptForSpecInfo(
          defaultAuthor,
          defaultRepository,
        );
        ...
        if (!isValid) {
          console.log(
            chalk.red(
              "Generated spec.json is not valid according to the schema.",
            ),
          );
          return;
        }
 ...
 */
async function promptForSpecInfo(
  ...
  try {
    const questions = [
      ...
      },
      {
        ...
        validate: (input: string) =>
          input.trim() !== "" || "Author name is required",
      },
      {
        type: "input",
        name: "authorName",
